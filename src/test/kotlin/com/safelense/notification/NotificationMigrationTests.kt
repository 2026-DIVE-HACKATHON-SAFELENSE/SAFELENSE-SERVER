...
class NotificationMigrationTests {
    ...
    fun `notifications migration defines storage ownership and query indexes`() {
        ...
        assertThat(sql).contains("read_at TIMESTAMP(6) WITH TIME ZONE NULL")
        assertThat(sql).contains("created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL")
        assertThat(sql).contains("FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE")
        assertThat(sql).contains("CREATE INDEX idx_notifications_user_id_id ON notifications (user_id, id)")
        assertThat(sql).contains("CREATE INDEX idx_notifications_user_id_read_at_id ON notifications (user_id, read_at, id)")
    }
}
